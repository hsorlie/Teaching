...
Legg merke til at før hvert svar står det et ettall, i firkantklammer. Med disse eksemplene får vi ett element til svar hver gang. Elementet "2", f.eks. Ettallet viser at 2 er det første elementet i svaret vårt. Hvis vi hadde fått en lang remse over flere linjer i svar, så står det et slikt tall i firkantklemmer for hver linje, som angir hvilket element det første på den linja er.
...
I R brukes en pil mot venstre som assignment operator: <- 
Du kan tenke på pila som at den tar det til høyre for seg, og putter det inn i det til venstre for seg.
...
Men vi skal videre. Å sitte og taste inn data manuelt med tibble() er ikke noe særlig effektivt. Jeg lover at det har vi gjort for siste gang. Fra nå skal vi importere data.

Vi skal ta pause, men først et par ord om filstruktur og working directory. Vi kan, med R-kode, henvise til filer som ligger lagret på maskinen vår, og vi kan selv opprette og lagre filer. Så når vi jobber så tenker alltid R at den befinner seg på et bestemt sted i filstrukturen vår, eller jobber ut fra et bestemt sted. Dette kalles working directory. Vi kan finne ut hva som er vårt nåværende working directory med funksjonen getwd().

```{r}

getwd()

#Hvis vi hadde lagret en fil med R-kode nå, uten å spesifisere hvor, så hadde den blitt lagret i denne banen. Vi kunne definert et annet sted, men da måtte vi spesifisert det med utgangspunkt i dette stedet.

# Vi kan også angi working directory med tilsvarende kommendo setwd(). Men det er litt knottete å holde på slik, og istedet så skal vi jobbe med Prosjekter, en funksjon i RStudio som tar seg av dette for oss. Det kan vi opprette etter pausen, men først skal jeg vise hvordan dere sletter ting fra Environment. Det gjør du med kommandoen rm(). Så kan du liste opp én eller flere objekter du vil slette.
...
Når vi nå fortsetter skal vi først opprette et Prosjekt. Prosjekter er en funksjonalitet i RStudio som gjør at vi slipper å bekymre oss for mye om working directory og slikt. Innenfor ett prosjekt kan du holde deg med så mange filer du bare vil, og lagre og styre på. Men du kan jobbe med flere prosjekter separat, i ulike RStudio-vinduer, samtidig om så. Environment holdes adskilt, og working directory settes til folderen for prosjektet. Jeg enbefaler at dere lager dere en mappe under Dokumenter, som dere f.eks. kaller R. Den mappen blir i så fall det naturlige stedet å lagre prosjektene deres.

Opprette nytt prosjekt...

Da har vi et tomt Environment, og Working Directory er satt til dette prosjektet, så det slipper vi å tenke på. I dette prosjektet kan vi nå åpne en ny fil. En script-fil.

Da velger dere File, New File, RScript. Det er lettere å jobbe med en scriptfil enn direkte i console fordi du her kan flikke og flikke på koden, og så trykke på Play når den ser klar ut, så sendes den til console.

## Data transformasjon

Vi skal i hovedsak herfra titte på tidyverse, et sett pakker som fungerer bra sammen for å lese, transformere og behandle data, og plotte. Tidyverse består som sagt av flere pakker, hver med et sett funksjoner. Men de er laget på en slik måte at de har felles syntaks, og felles oppbygning av argumenter.

Vi begynner med å laste et datasett, så vi har noe å jobbe med. Det finner vi i pakken "nycflights13".
...
Alle disse funksjonene fungerer på følgende måte: De tar et objekt, som et datasett, som første argument. For alle påfølgende argumenter sier du hva du vil gjøre med datasettet, ved å bruke variabelnavn. Outputen av funksjonen er et nytt datasett.

Og dette fungerer sammen med funksjonen `group_by()`. Den gjør at hver av de andre funksjonene fungerer innad i en hver definerte gruppe av datasettet, og ikke bare på datasettet som helhet. Si f.eks. at du bruker summarise() til å lage snittscorer for variabler. Da vil group_by() sørge for at snittet ikke blir for datasettet som helhet, for innad for hver gruppe basert på en annen variabel.
...
Vi kan begynne med filter(). La oss si at vi er interessert kun i flyene som forlot NY på kvinnedagen:
```{r}
...
# Ok, det var filter. Neste er select(). Den er ganske basic. Den bare plukker variablene du er interessert i. Men den er ganske praktisk hvis du skal sakse ned et stort datasett til noe mer håndterbart for å gjøre analyser.
...
# Og så mutate(). Den er supernyttig. La oss først lage et lite datasett med noen få variabler:
...
#Neste er summarise(). Den kollapser en variabel ned til én verdi. 

summarise(flights, mean_delay = mean(dep_delay))

#Yes! Nå fikk vi NA. Så dette er et fint tidspunkt å ta det på. NA står for "not available", og det er slik missing verdier ser ut i R. Tolkningen er altså at der det står NA vet ikke R hva det egentlig skulle stått. Og NA er svært smittsomt, fordi nesten alle operasjoner som involverer én NA også får NA til svar. Et eksempel:
x <- c(1, 2, 3, 4, 5)
y <- 10
z <- NA
...
# Dette er virkelig noe å ha i bakhodet, f.eks. når man regner summescorer av items, for å få scorer på variabler. For det betyr at hvis du ikke foretar deg noe så blir hele summescorer NA, eller missing, for en respondent hvis én av itemene er NA. Hvis det er et måleverktøy der mange items laster på én faktor, så kan det bli ganske stygt. Jeg har vært borti dette med f.eks. NEO, som har 240 items som laster på 30 personlighetsfasetter, som igjen laster på 5 personlighetsfaktorer. Det var et datasett på et par tusen, og det var ikke veldig mye missing, men noe. Og der det var missing var det kanskje slik at en respondent hadde missing på én item av 240, midt inni der et sted. Og hvis én av åtte items i en fasett er missing så blir fasetten missing. Og hvis én av seks fasetter i en faktor er missing, så blir faktoren missing. Noen ganger må det jo bare bli sånn, men man må kanskje i det minste kunne diskutere hvor mange items man kan tåle er missing uten at det går ut over konstruktvaliditeten hvis man tar snittet av de resterende.

# Jeg vet ikke hva som er riktig sånn sett, men den tekniske løsninga er enkel. Vi må ofte be R eksplisitt om å ignorere NAer.

summarise(flights, mean_delay = mean(dep_delay, na.rm = TRUE))

# Da får vi et svar. Men denne funksjonen er riktignok ikke veldig nyttig med mindre den kombineres med group_by, så da tar vi den.
...
#Ser dere at de ser like ut? Like dimensjoner, samme variabler. Men den ene har angitt "groups". 16 ulike verdier av carrier. Og alle andre funksjoner vi gjør med dette datasettet, blir gjort for hver gruppe av datasettet. Iallefall alle tidyverse-funksjoner respekterer denne grupperingen.
...
Jeg foreslår at dere legger de under en mappe som heter "data" der dere har prosjektet dere. Slik...
...
Det finnes også en del pakker som bygger videre på ggplot. De tar enkle argumenter og gir dem videre til ggplot for å lage en del spesialplot, som f.eks, interaksjonsplot eller Johnson-Neyman plot. Sjekk ut pakkene ggstatsplot, f.eks., eller interactions.
...
#La oss si at vi er interessert i flyvninger fra NYs JFK, til Los Angeles. Og vi er interessert i hvorvidt det er forskjeller i forsinkelser på tvers av f.eks. vår og høst. Og vi vil se forskjeller på ulike flyselskaper.

flights %>%
        subset(origin == "JFK" & dest == "LAX") %>%
        subset(month == 3:5 | month == 9:11) %>%
...
Jeg skal ikke demonstrere ytterligere hvordan denne koden hadde sett ut uten bruk av pipes. Jeg tipper dere ser selv at det hadde vært ganske mye mer rotete.

# RMarkdown

Hvor mange har hørt om Markdown?

Markdown er genialt. Det er 



# SÅ langt har jeg kommet til nå...
